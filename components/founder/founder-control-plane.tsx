...
function FounderControlPlaneContent() {
  ...
  const [runningAction, setRunningAction] = useState<'brief' | 'options' | 'blueprint' | 'ignition' | 'deploy' | null>(
    null
  );
    ...
    (async () => {
      try {
        ...
        if (cap && typeof cap === 'object' && !Array.isArray(cap)) {
          ...
          setStatus({
            kind: 'success',
            text: 'Form filled from your Avril chat (Convex draft). Review and save intake when ready.',
          });
        } else {
          setStatus({
            kind: 'info',
      ...
      router.replace('/founder/control-plane', { scroll: false });
    })();
  ...
  const deployMode =
    state.deploymentJob?.target === 'mock' ? 'mock' : state.deploymentJob?.target === 'live' ? 'live' : 'n/a';
  ...
  return (
    <div className="font-sans mx-auto w-full max-w-6xl space-y-5 px-2">
      <SectionTitle
        title="Founder Control Plane"
        subtitle="Operator tools — manual intake, brief, options, blueprint, ignition, and deploy."
      />
      ...
      <div className="grid grid-cols-1 gap-4">
        <Card className="rounded-2xl p-5">
          <h3 className="mb-3 font-heading font-semibold">Founder Idea</h3>
          <form onSubmit={handleSubmit} className="space-y-3 text-sm">
            <input
              className="w-full rounded-xl border border-border bg-surface px-3 py-2"
              placeholder="Founder name"
              value={form.founderName}
              onChange={(e) => setForm((s) => ({ ...s, founderName: e.target.value }))}
            />
            <input
              className="w-full rounded-xl border border-border bg-surface px-3 py-2"
            ...
            />
            <textarea
              className="min-h-24 w-full rounded-xl border border-border bg-surface px-3 py-2"
              placeholder="Raw idea (required)"
              value={form.rawIdea}
              onChange={(e) => setForm((s) => ({ ...s, rawIdea: e.target.value }))}
            />
            <input
              className="w-full rounded-xl border border-border bg-surface px-3 py-2"
              placeholder="Target user"
              value={form.targetUser}
              onChange={(e) => setForm((s) => ({ ...s, targetUser: e.target.value }))}
            />
            <input
              className="w-full rounded-xl border border-border bg-surface px-3 py-2"
              placeholder="Problem"
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Monetization preference"
                value={form.monetizationPreference}
                onChange={(e) => setForm((s) => ({ ...s, monetizationPreference: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Business model preference"
                value={form.businessModelPreference}
                onChange={(e) => setForm((s) => ({ ...s, businessModelPreference: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Automation level"
                value={form.desiredAutomationLevel}
                onChange={(e) => setForm((s) => ({ ...s, desiredAutomationLevel: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Risk tolerance"
                value={form.riskTolerance}
                onChange={(e) => setForm((s) => ({ ...s, riskTolerance: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Skills/resources"
                value={form.skillsResources}
                onChange={(e) => setForm((s) => ({ ...s, skillsResources: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Time available"
                value={form.timeAvailable}
                onChange={(e) => setForm((s) => ({ ...s, timeAvailable: e.target.value }))}
              />
              <input
                className="w-full rounded-xl border border-border bg-surface px-3 py-2"
                placeholder="Country"
            ...
            <input
              ...
              onChange={(e) => setForm((s) => ({ ...s, channelPreferences: e.target.value }))}
            />
        ...
        <Card className="space-y-3 rounded-2xl p-5">
          <h3 className="font-heading font-semibold">Company Options</h3>
          ...
          <div className="space-y-2">
            {state.options.length === 0 ? (
              ...
              state.options.map((option) => {
                ...
                return (
                  <div key={option._id} className="rounded-lg border border-white/10 px-3 py-2">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm capitalize text-white">{option.profile}</p>
                        <p className="truncate text-xs text-muted">{option.businessThesis}</p>
                      </div>
                      <Button
                        className="text-xs"
        ...
        <Card className="space-y-3 rounded-2xl p-5">
          <h3 className="font-heading font-semibold">Selected Route / Blueprint</h3>
          <div className="space-y-2 text-sm">
            <p>
              <span className="text-muted">Selected route:</span> {state.selectedRoute?.selectedProfile || 'none'}
            </p>
            <p>
              <span className="text-muted">Blueprint:</span>{' '}
              {state.blueprint ? `v${state.blueprint.version}` : 'missing'}
            </p>
            <p>
              <span className="text-muted">Ignition:</span> {state.ignitionConfig ? 'ready' : 'missing'}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
        ...
        <Card className="space-y-3 rounded-2xl p-5">
          <h3 className="font-heading font-semibold">Deployment Status</h3>
          <div className="space-y-2 text-sm">
            <p>
              <span className="text-muted">Deployment status:</span> {state.deploymentJob?.status || 'none'}
            </p>
            <p>
              <span className="text-muted">Runtime status:</span> {state.runtimeInstance?.status || 'none'}
            </p>
            <p>
              <span className="text-muted">Runtime URL:</span> {state.runtimeInstance?.endpointUrl || 'n/a'}
            </p>
            <p>
              <span className="text-muted">External deployment ID:</span>{' '}
              {state.deploymentJob?.externalDeploymentId || 'n/a'}
            </p>
            <p>
              <span className="text-muted">Deploy mode:</span> {deployMode}
            </p>
          </div>
          <Button
            className="text-xs"
        ...
        <Card className="space-y-3 rounded-2xl p-5">
          <h3 className="font-heading font-semibold">Recent Events / Identity</h3>
          <div className="space-y-1 text-xs">
            <p>
              <span className="text-muted">Connected identity:</span> {address || 'not connected'}
            </p>
            <p>
              <span className="text-muted">Pending approvals:</span> {state.pendingApprovals.length}
            </p>
          </div>
          <div className="max-h-56 space-y-2 overflow-y-auto text-xs">
            {deploymentEvents.length === 0 ? (
              ...
              deploymentEvents.map((event) => (
                <div key={event._id} className="rounded-lg border border-white/10 px-2 py-1.5">
...
export function FounderControlPlane() {
  return (
    <Suspense fallback={<div className="font-sans p-8 text-sm text-muted">Loading…</div>}>
      <FounderControlPlaneContent />
    </Suspense>
  );
}
