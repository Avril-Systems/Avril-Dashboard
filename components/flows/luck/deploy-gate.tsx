'use client';

import { useCallback, useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { Loader2, Wallet } from 'lucide-react';
import { useWaaP } from '@/src/components/WaaPProvider';
import { establishWalletSession, fetchWalletSession } from '@/src/lib/establishWalletSession';
import { clearPersistedLuckSelection, persistLuckSelection } from '@/src/lib/luckIntake';
import { MarketingBrandButton } from '@/components/marketing/marketing-brand-button';
import { PaymentModule } from './payment-module';
import type { Opportunity } from './types';

type GatePhase = 'sign-in' | 'linking' | 'payment' | 'complete';

type DeployGateProps = {
  opportunity: Opportunity;
  onRestart: () => void;
};

export function DeployGate({ opportunity, onRestart }: DeployGateProps) {
  const router = useRouter();
  const { isReady, login } = useWaaP();
  const [phase, setPhase] = useState<GatePhase>('sign-in');
  const [ideaId, setIdeaId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);

  const saveSelection = useCallback(async () => {
    const res = await fetch('/api/founder/luck-intake', {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ opportunity }),
    });
    const data = (await res.json()) as {
      ok?: boolean;
      ideaId?: string;
      error?: { message?: string };
    };
    if (!res.ok || !data.ok || !data.ideaId) {
      throw new Error(data.error?.message || 'Could not link your selection to your account.');
    }
    setIdeaId(data.ideaId);
    return data.ideaId;
  }, [opportunity]);

  const resumeFromSession = useCallback(async () => {
    const session = await fetchWalletSession();
    if (!session) {
      setPhase('sign-in');
      return;
    }

    if (session.luckIdeaId) {
      setIdeaId(session.luckIdeaId);
      if (session.plan) {
        setPhase('complete');
      } else {
        setPhase('payment');
      }
      return;
    }

    setPhase('linking');
    try {
      await saveSelection();
      setPhase('payment');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to save selection');
      setPhase('sign-in');
    }
  }, [saveSelection]);

  useEffect(() => {
    persistLuckSelection(opportunity);
  }, [opportunity]);

  useEffect(() => {
    if (!isReady) return;
    void resumeFromSession();
  }, [isReady, resumeFromSession]);

  async function handleSignIn() {
    setBusy(true);
    setError(null);
    try {
      await login();

      let walletAddress = '';
      if (!walletAddress) {
        try {
          const accounts = await window.waap?.request?.({ method: 'eth_requestAccounts' });
          walletAddress = Array.isArray(accounts) ? String(accounts[0] || '').toLowerCase() : '';
        } catch {
          const ethAccounts = await (window as Window & { ethereum?: { request: (args: { method: string }) => Promise<unknown> } }).ethereum?.request?.({
            method: 'eth_requestAccounts',
          });
          walletAddress = Array.isArray(ethAccounts) ? String(ethAccounts[0] || '').toLowerCase() : '';
        }
      }

      if (!walletAddress) {
        throw new Error('Wallet connected but no address was returned. Try again.');
      }

      setPhase('linking');
      await establishWalletSession(walletAddress);
      await saveSelection();
      setPhase('payment');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Sign-in failed');
      setPhase('sign-in');
    } finally {
      setBusy(false);
    }
  }

  function handlePaymentComplete() {
    clearPersistedLuckSelection();
    setPhase('complete');
    const target = ideaId ? `/home?ideaId=${encodeURIComponent(ideaId)}` : '/home';
    router.push(target);
  }

  if (phase === 'linking') {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-4 py-12 text-center">
        <Loader2 className="h-8 w-8 animate-spin text-brand" />
        <p className="text-muted-foreground">Linking {opportunity.name} to your wallet…</p>
      </div>
    );
  }

  if (phase === 'payment') {
    return (
      <PaymentModule companyName={opportunity.name} ideaId={ideaId ?? undefined} onComplete={handlePaymentComplete} />
    );
  }

  if (phase === 'complete') {
    return (
      <div className="mx-auto max-w-xl space-y-6 py-12 text-center">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-brand/30 bg-brand/10 text-2xl text-brand">
          ✓
        </div>
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold md:text-3xl">{opportunity.name} is ready</h2>
          <p className="text-muted-foreground">Opening your dashboard…</p>
        </div>
      </div>
    );
  }

  return (
    <div className="mx-auto max-w-xl space-y-6 py-12 text-center">
      <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-brand/30 bg-brand/10 text-2xl text-brand">
        ✓
      </div>
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold md:text-3xl">{opportunity.name} queued for deploy</h2>
        <p className="text-muted-foreground">
          Sign in with your wallet to save this blueprint to your account, then choose a plan to launch your agent team.
        </p>
      </div>

      {error && <p className="text-sm text-rose-400">{error}</p>}

      <div className="flex flex-col items-center gap-3">
        <MarketingBrandButton
          label={busy ? 'Connecting…' : 'Sign in to continue'}
          onClick={() => void handleSignIn()}
        />
        <p className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
          <Wallet size={12} />
          Human.tech WaaP · wallet signature secures your session
        </p>
      </div>

      <button type="button" onClick={onRestart} className="text-sm text-brand hover:underline">
        Generate new opportunities
      </button>
    </div>
  );
}
